...
const HomeCoverSection = ({ post }) => {
  return (
    <article className="relative h-[80vh] max-sm:mt-4 flex flex-col items-start justify-end rounded-3xl">
      <div
      ...
      />
      <div className="absolute bottom-4 w-3/4 p-16 max-sm:px-6 max-sm:py-20 max-sm:w-full flex items-start justify-center flex-col text-light">
        <Tag
          link={`categories/${post?.category?.$id}`}
          name={post?.category?.name}
        />
        <Link className="mt-6" to={`/blog/${post?.$id}`}>
          <h1 className="font-black capitalize text-4xl max-sm:text-2xl cursor-pointer">
            <span
